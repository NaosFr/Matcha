...
/////////VERFICATION
if ($data_user['bio'] == "" || $data_user['last_name'] == "" ||
	$data_user['first_name'] == "" || $data_user['sexe'] == 0 ||
	$data_user['orientation'] == 0 || $data_user['age'] == "" ||
	!check_image($_SESSION['id'])) {
	echo '<p class="error">PROFIL NOT COMPLETED</p>';
	exit ;
}
...
$users = [];
$req = $bdd->prepare('SELECT * FROM users WHERE age != "" AND bio != "" AND confirm = 1 AND sexe != 0 AND orientation != 0 AND id_user != ?');
...
foreach ($users as $key => $profil) {
	if ($data_user['orientation'] & $profil['gender'] == $profil['gender'] && $profil['orientation'] & $data_user['gender'] == $data_user['gender'])
		{
			$match_user[] = $profil;
			unset($users[$key]);
		}
...
$req = $bdd->prepare('SELECT * FROM tags INNER JOIN assoc ON tags.id_tag = assoc.id_tag AND assoc.id_user =:id');
$req->bindParam(':id', $_SESSION['id'], PDO::PARAM_INT);
$req->execute();
...
foreach ($users as $key => $profil) {
	if ($profil['score'] >= $data_user['score'] - 100 &&
	$profil['score'] <= $data_user['score'] + 100 )
	{
		$match_user[] = $profil;
		unset($users[$key]);
	}
...
foreach ($match_user as $key => $profil)
{
	...
	if ($index >= $nbr_picture) {
		...
		$data['last_name'] = strtoupper($data['last_name']);
				...
				echo '	<div class="user_card">
							...
							<p class="distance">30 <span class="blue">Km<span></p>
